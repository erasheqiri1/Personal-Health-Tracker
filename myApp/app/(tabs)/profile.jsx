...
export default function Profile() {
  ...
  // ✅ Lexo gjendjen e notifications nga storage
  useEffect(() => {
    (async () => {
      try {
  ...
  const handleToggleWaterReminder = async (nextValue) => {
    ...
    try {
      ...
      // ktheje switch-in në gjendjen e mëparshme
    ...
    >
      ...
      <View style={[styles.notifCard, { backgroundColor: COLORS.card }]}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ marginTop: 4, color: COLORS.textDark }}>
            Çdo 1 orë: “Mos harroni të pini ujë”
          </Text>
        </View>

        <Switch
          value={waterReminderOn}
          onValueChange={(val) => handleToggleWaterReminder(val)}
          disabled={notifBusy}
        />
      </View>

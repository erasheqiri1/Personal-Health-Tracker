...
export default function Profile() {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.image) setAvatarUri(data.image);
        } else {
  ...
  useEffect(() => {
    (async () => {
      try {
        const enabled = await getWaterReminderState();
        setWaterReminderOn(enabled);
      } catch (e) {
        console.log("load reminder state error:", e);
      }
    })();
  }, []);

  const handleToggleWaterReminder = async (nextValue) => {
    if (notifBusy) return;

    setNotifBusy(true);
    try {
      if (nextValue) {
        const res = await scheduleHourlyWaterReminder();
        if (!res.ok) {
          Alert.alert(
            "Leja për njoftime",
            "S’u aktivizua sepse nuk u lejua permission. Shko te Settings > Notifications dhe lejo njoftimet."
          );
          setWaterReminderOn(false);
          return;
        }
        setWaterReminderOn(true);
        Alert.alert("Sukses", "U aktivizua kujtesa çdo 1 orë: “Mos harroni të pini ujë”.");
      } else {
        await cancelHourlyWaterReminder();
        setWaterReminderOn(false);
        Alert.alert("OK", "U çaktivizua kujtesa.");
      }
    } catch (e) {
      console.log("toggle reminder error:", e);
      Alert.alert("Gabim", "Diçka dështoi me njoftimet. Provo prapë.");
      setWaterReminderOn((prev) => prev);
    } finally {
      setNotifBusy(false);
    }
  };
  ...
  const handleImagePicked = async (result) => {
    try {
      ...
      await setDoc(userRef, { image: base64Img }, { merge: true });
  ...
  const deleteImage = async () => {
    try {
      ...
      await setDoc(userRef, { image: null }, { merge: true });
  ...
  const openImageOptions = () => {
    if (!avatarUri) {
      Alert.alert("Add profile picture", "Zgjidh një opsion", [
        { text: "Choose from library", onPress: chooseFromLibrary },
        { text: "Take photo", onPress: takePhoto },
        { text: "Cancel", style: "cancel" },
      ]);
    } else {
      Alert.alert("Edit profile picture", "Zgjidh një opsion", [
        { text: "Choose from library", onPress: chooseFromLibrary },
        { text: "Take photo", onPress: takePhoto },
  ...
  const handleSave = async () => {
    try {
      ...
      const parsedWeight = form.pesha ? Number(String(form.pesha).replace(",", ".")) : null;
      const parsedHeight = form.gjatesia ? Number(String(form.gjatesia).replace(",", ".")) : null;
      ...
      await updateProfile(user, { displayName: `${form.emri} ${form.mbiemri}` });
  ...
  const linkSocialAccount = async (provider, providerName) => {
    try {
      ...
      if (Platform.OS !== "web") {
        Alert.alert("Info", `Lidhja me ${providerName} funksionon vetëm në web.`);
        return;
      }
      ...
      if (e.code === "auth/provider-already-linked") {
        ...
        Alert.alert("Gabim", `Kjo llogari ${providerName} është e lidhur me një përdorues tjetër.`);
      } else if (e.code === "auth/requires-recent-login") {
        Alert.alert("Gabim", "Lidhja kërkon që të kycësh përsëri. Dil dhe hy prapë, pastaj provo përsëri.");
      } else {
  ...
  const handleLinkGoogle = () => linkSocialAccount(new GoogleAuthProvider(), "Google");
  const handleLinkMicrosoft = () => linkSocialAccount(new OAuthProvider("microsoft.com"), "Microsoft");
  const handleLinkGitHub = () => linkSocialAccount(new GithubAuthProvider(), "GitHub");
  const handleLinkFacebook = () => linkSocialAccount(new FacebookAuthProvider(), "Facebook");

  if (loading) {
    return (
      <View style={[styles.loading, { backgroundColor: COLORS.page }]}>
        <Text style={[styles.loadingText, { color: COLORS.textDark }]}>Duke ngarkuar profilin...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <TouchableOpacity style={[styles.button, { backgroundColor: COLORS.green }]} onPress={handleSave} disabled={saving}>
        <Text style={[styles.buttonText, { color: COLORS.page }]}>{saving ? "Duke ruajtur..." : "Ruaj Ndryshimet"}</Text>
      </TouchableOpacity>

      <Text style={styles.linkTitle}>Dëshironi të lidheni llogarin tuaj aktuale me:</Text>
...
/>
      ...
      <TouchableOpacity style={[styles.logoutButton, { borderColor: COLORS.green }]} onPress={handleLogout}>
        <Text style={[styles.logoutText, { color: COLORS.textDark }]}>Dil nga llogaria</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { width: "100%", borderRadius: 12, padding: 20, marginBottom: 16 },

  row: { flexDirection: "row", justifyContent: "space-between", marginVertical: 6, alignItems: "center" },
  ...
  linkTitle: { marginTop: 16, marginBottom: 8, fontSize: 16, fontWeight: "600", color: COLORS.textDark },
  ...
  logoutButton: { marginTop: 12, padding: 10, borderWidth: 2, borderRadius: 10, width: "100%", alignItems: "center" },

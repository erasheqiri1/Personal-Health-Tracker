...
//   // NOTE: po e lë siç e ke. Nëse useWeather e mbështet lat/lon, mund ta përmirësosh më vonë.
//   // const { weather, weatherLoading, weatherError } = useWeather();
...
//   // Nëse është ma larg se kaq, mos e sugjero si “afër”
...
//     // cache sipas afërsisë (0.003 ~ 300m), që mos me i rënë Overpass shpesh
...
//     if (temperature == null) {
//       return { text: "", poiType: null, mode: "none" };
//     }

//     const d = descriptionEn.toLowerCase();
//     const isRain =
//       d.includes("rain") || d.includes("storm") || d.includes("drizzle");
//     const isSnow = d.includes("snow");
//     const isVeryCold = temperature < 0;
//     const isCold = temperature >= 0 && temperature < 8;
//     const isHot = temperature > 28;
...
//       return {
//         text:
//           "Ka shi/borë – bëj ushtrime brenda (home workout, yoga ose stretching).",
//         poiType: null,
//         mode: "home",
//       };
//     }
...
//       return {
//         text:
//           "Shumë ftohtë jashtë – temperaturat janë shumë të ulëta. Ushtroni në shtëpi.",
//         poiType: null,
//         mode: "home",
//       };
//     }
...
//       return {
//         text:
//           "Moti shumë i nxehtë – ec në mëngjes ose mbrëmje dhe pi shumë uj gjatë ditës.",
//         poiType: null,
//         mode: "light",
//       };
//     }
...
//       return {
//         text:
//           "Pak ftohtë – rekomandohet fitnesi më i afërt (ose ushtrime brenda).",
//         poiType: "gym",
//         mode: "gym",
//       };
//     }
...
//       return {
//         text:
//           "Kushtet perfekte për ecje 30–40 minuta jashtë ose një vrap të lehtë.",
//         poiType: "park",
//         mode: "outdoor",
//       };
//     }
...
//       return {
//         text:
//           "Moti i mirë për aktivitet jashtë – bën një shëtitje të lehtë ose çiklizëm.",
//         poiType: "park",
//         mode: "outdoor",
//       };
//     }
...
//     return {
//       text:
//         "Zgjidh një aktivitet të lehtë – kombinim ecje jashtë dhe ushtrimesh brenda.",
//       poiType: null,
//       mode: "light",
//     };
//   };
...
//         // nëse s’kemi coords ose s’kemi nevojë për POI
...
//           setPoiError(
//             type === "park"
//               ? "Nuk u gjet park afër."
//               : "Nuk u gjet fitnes afër."
//           );
//           return;
//         }

//         // nëse është larg, konsideroje si “s’ka afër”
//         if (nearest.distanceKm > FAR_KM_THRESHOLD) {
//           setNearestPoi(null);
//           setPoiError(
//             type === "park"
//               ? "Nuk keni park afër — ushtroni në shtëpi."
//               : "Nuk keni fitnes afër — ushtroni në shtëpi."
//           );
//           return;
//         }
...
//     // nëse është outdoor/gym dhe kemi nearestPoi, bëje sugjerimin “me emër”
...
//     // nëse s’kemi poi por kemi poiError (p.sh. larg), shfaq atë mesazh
//     if (plan.poiType && poiError) return poiError;

//     return plan.text;
//   })();

//   // ======== Sleep handlers (siç i ke) ========
//   const handleStartSleep = async () => {
//     const now = new Date();
//     setSleepStart(now);
...
//         await setDoc(
//           sleepDocRef,
//           { startTime: now.toISOString(), endTime: null, minutes: 0 },
//           { merge: true }
//         );
//       } catch (e) {
//         console.log("Start sleep Firestore error:", e);
...
//           await setDoc(
//             sleepDocRef,
//             { endTime: now.toISOString(), minutes: mins },
//             { merge: true }
//           );
//         } catch (e) {
//           console.log("Stop sleep Firestore error:", e);
...
//   // ======== Recipe search handlers (siç i ke) ========
//   const handleSearchTextChange = (text) => {
//     setSearchText(text);
//     if (text.trim() === "") {
//       setSearchRecipe(null);
...
//       }
//     : {
//         latitude: 42.6629, // fallback (Prishtinë approx)
//         longitude: 21.1655,
//         latitudeDelta: 0.05,
//         longitudeDelta: 0.05,
//       };
...
//         {/* MOTI - E LËNJ NJËJTË SIÇ E KE (screenshot) */}
// <View style={[styles.cardBase, styles.weatherCard]}>
//   <View style={styles.weatherHeaderRow}>
//     <Text style={styles.weatherTitle}>Moti sot</Text>
//     {cityName && (
...
//           {locStatus === "loading" && (
//             <Text style={styles.loadingText}>Duke marrë lokacionin...</Text>
//           )}
...
//           {locStatus === "error" && (
//             <Text style={styles.errorText}>
//               Gabim gjatë marrjes së lokacionit.
//             </Text>
//           )}
...
//                       title="Ti je këtu"
//                       description="Lokacioni yt aktual"
//                     />
//                   )}
...
//                   {lineCoords && (
//                     <Polyline coordinates={lineCoords} strokeWidth={3} />
//                   )}
//                 </MapView>
//               </View>

//               {/* Teksti poshtë hartës */}
//               {nearestPoi ? (
...
//               ) : (
//                 <Text style={styles.mapNote}>
//                   {plan.poiType && poiError
//                     ? poiError
//                     : "S’ka destinacion të sugjeruar për momentin."}
//                 </Text>
//               )}
//             </>
//           )}
...
//         {/* PJESËT TJERA - i kam lënë siç i ke */}
//         <View style={[styles.cardBase, styles.cardSmall]}>
//           <Text style={styles.title}>Gjumi</Text>
//           <View style={styles.sleepRow}>
//             <Text style={styles.moon}>🌙</Text>
...
//           {isSleeping && (
//             <Text style={styles.sleepStatusText}>Gjumi është duke u matur...</Text>
//           )}
//         </View>
...
//           {!searchRecipe && recipeLoading && (
//             <Text style={styles.loadingText}>Duke u ngarkuar receta...</Text>
//           )}
//           {!searchRecipe && !!recipeError && (
//             <Text style={styles.errorText}>{recipeError}</Text>
//           )}
